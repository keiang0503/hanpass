...
export const View2Subsection = (): JSX.Element => {
  return (
    <section className="flex flex-col items-start gap-6 px-6 py-8 w-full">
      <div className="flex flex-col items-start gap-4 w-full">
        <div className="flex items-center gap-1 w-full">
          ...
          <Button
            variant="ghost"
            className="h-auto px-2.5 py-2 bg-[#f4f7fd] rounded-lg hover:bg-[#e8eef9]"
          >
              ...
              <div className="flex flex-col items-start gap-3 p-4 w-full bg-[#f7f7f8] rounded-lg">
                {limitDetails.map((item, index) => (
                  <div key={index} className="flex items-center gap-1 w-full">
                    ...
                    <div
                      className={`flex-1 mt-[-1.00px] font-caption-title-13-r font-[number:var(--caption-title-13-r-font-weight)] ${item.valueColor} text-[length:var(--caption-title-13-r-font-size)] text-right tracking-[var(--caption-title-13-r-letter-spacing)] leading-[var(--caption-title-13-r-line-height)] [font-style:var(--caption-title-13-r-font-style)]`}
                    >
                      {item.value}
                    </div>
                  </div>
